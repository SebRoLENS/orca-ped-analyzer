...
Main outputs:
  ...
  - CSV assignment and PED/TED contribution tables; TED runs also export the full PED/KED/TED matrices
  - one native Avogadro CJSON containing all harmonic normal modes
  - a manifest describing every generated output
...
def full_ted_matrix_for_mode(F, Ginv, d_normalized, mode_lambda):
    """Return full PED, KED and TED matrices for one harmonic mode.

    V_mn = D_m F_mn D_n / lambda
    T_mn = D_m (G^-1)_mn D_n
    E_mn = 1/2 (V_mn + T_mn)

    D is kinetically normalized so that D^T G^-1 D = 1. Each complete
    matrix therefore sums to unity. Off-diagonal coupling terms are retained
    and can be negative.
    """
    F=np.asarray(F,dtype=float)
    Ginv=np.asarray(Ginv,dtype=float)
    d=np.asarray(d_normalized,dtype=float).reshape(-1)
    lam=float(mode_lambda)
    if F.shape != Ginv.shape or F.shape != (d.size,d.size):
        raise ValueError("Incompatible F, G^-1 and displacement dimensions for full TED.")
    if not np.isfinite(lam) or lam <= 0:
        raise ValueError("Full TED requires a positive finite modal lambda.")
    outer=np.outer(d,d)
    potential=(outer*F)/lam
    kinetic=outer*Ginv
    total=0.5*(potential+kinetic)
    for name,matrix in (("PED",potential),("KED",kinetic),("TED",total)):
        if np.any(~np.isfinite(matrix)):
            raise ValueError(f"Non-finite values encountered in full {name} matrix.")
        if not np.isclose(matrix.sum(),1.0,rtol=1e-8,atol=1e-8):
            raise ValueError(f"Full {name} matrix does not sum to unity.")
    return potential,kinetic,total


def describe_generated_output(path, distribution_label):
    """Concise description for one file listed in the run manifest."""
    name=Path(path).name.lower()
    if name.endswith("_summary.csv"):
        return f"Per-mode summary with harmonic/VPT2 frequencies, intensities, assignments and grouped {distribution_label} contributions."
    if name.endswith("_families.csv"):
        return f"Mode contributions grouped into topological and generic vibrational families using {distribution_label}."
    if name.endswith("_ted_full.csv"):
        return "Full mode-resolved PED, KED and TED matrices in internal coordinates, including off-diagonal coupling terms; entries may be negative."
    if name.endswith("_ped.csv"):
        return "Primitive internal-coordinate diagonal PED percentages, phases, displacements and diagonal force constants."
    if name.endswith("_ted.csv"):
        return "Primitive internal-coordinate diagonal TED percentages used for automatic assignment, with F diagonal, G^-1 diagonal and modal lambda."
    if name.endswith("_ir_transitions.csv"):
        return "Unified table of harmonic fundamentals and available anharmonic VPT2 transitions with frequencies, intensities and assignments."
    if name.endswith("_vpt2_bands.csv"):
        return "Parsed VPT2 overtone and combination-band table with available anharmonic intensities and assignments."
    if name.endswith("_fermi.txt"):
        return "Fermi-resonance information extracted from the ORCA VPT2 output."
    if name.endswith("_ir_fundamentals.dat"):
        return "Broadened IR spectrum containing fundamental transitions only."
    if name.endswith("_ir_anharmonic.dat"):
        return "Broadened IR spectrum containing anharmonic-only overtone and combination transitions."
    if name.endswith("_ir_complete.dat"):
        return "Broadened complete IR spectrum combining fundamentals with available overtone and combination bands."
    if name.endswith("_avogadro_vibrations.cjson"):
        return "Avogadro CJSON containing geometry, harmonic frequencies/intensities, connectivity and all harmonic normal-mode vectors."
    if name.endswith("_manifest.txt"):
        return "Run manifest recording inputs, settings, analysis status and a brief description of every generated output."
    return "Additional output generated by ORCA PED Analyzer during this run."
...
def main():
    ...
    ginv_full=distribution_diag["G_inverse"]
    d_ted_normalized=distribution_diag["D_normalized"]
    ...
    if not args.no_csv:
        ...
        full_ted_file=None
        if distribution_key == "ted":
            full_ted_file=str(p)+"_ted_full.csv"
            with open(full_ted_file,"w",newline="") as fh:
                w=csv.writer(fh)
                w.writerow([
                    "mode","harmonic_frequency_cm-1","assignment",
                    "row_index","column_index","row_internal_coordinate","column_internal_coordinate",
                    "potential_contribution_percent","kinetic_contribution_percent",
                    "total_energy_contribution_percent","diagonal"
                ])
                for col,mi in enumerate(vib):
                    pm,km,tm=full_ted_matrix_for_mode(
                        F,ginv_full,d_ted_normalized[:,col],mode_lambda[col]
                    )
                    for r,ric in enumerate(ics):
                        rlabel=ic_label(ric,elems)
                        for c,cic in enumerate(ics):
                            w.writerow([
                                int(mi),float(freqs[mi]),mode_assignments[int(mi)],
                                r+1,c+1,rlabel,ic_label(cic,elems),
                                100.0*float(pm[r,c]),100.0*float(km[r,c]),
                                100.0*float(tm[r,c]),bool(r==c)
                            ])

        generated_files.extend([str(Path(sfile).resolve()),str(Path(ffile).resolve()),str(Path(dfile).resolve())])
        if full_ted_file is not None:
            generated_files.append(str(Path(full_ted_file).resolve()))
        print(f"\nWrote: {sfile}")
        print(f"Wrote: {ffile}")
        print(f"Wrote: {dfile}")
        if full_ted_file is not None:
            print(f"Wrote: {full_ted_file}")
    ...
    manifest_entries=list(generated_files)+[str(manifest_path.resolve())]
    manifest_lines.extend(["",f"generated_files: {len(manifest_entries)}"])
    for x in manifest_entries:
        manifest_lines.append(
            f"  {Path(x).name} - {describe_generated_output(x,distribution_label)}"
        )
